...
OPPAIAPI
char *oppai_version_str() { return OPPAI_VERSION_STRING; }
...
float v2f_len(float *v) { return (float)sqrt(v[0] * v[0] + v[1] * v[1]); }

float v2f_dot(float *a, float *b) { return a[0] * b[0] + a[1] * b[1]; }

/* https://www.doc.ic.ac.uk/%7Eeedwards/compsys/float/nan.html */

int is_nan(float b) {
  unsigned *p = (void *)&b;
  return ((*p > 0x7F800000 && *p < 0x80000000) ||
...
int slice_len(slice_t *s) { return (int)(s->end - s->start); }
 ...
 * ERR_TRUNCATED
 */
int slice_split(slice_t *s, char *separator_list, slice_t *arr, int nmax,
                int *err) {
  int res = 0;
  char *p = s->start;
  char *pprev = p;
...
#define oppai_array_reserve(arr, n)                                            \
  oppai_array_reserve_i(n, oppai_array_unpack(arr))

#define oppai_array_free(arr) oppai_array_free_i(oppai_array_unpack(arr))

#define oppai_array_alloc(arr)                                                 \
  (oppai_array_reserve((arr), (arr)->len + 1) ? &(arr)->data[(arr)->len++] : 0)

#define oppai_array_append(arr, x)                                             \
  (oppai_array_reserve((arr), (arr)->len + 1)                                  \
       ? ((arr)->data[(arr)->len++] = (x), 1)                                  \
       : 0)

/* internal helpers, not to be used directly */
#define oppai_array_unpack(arr)                                                \
  &(arr)->cap, &(arr)->len, (void **)&(arr)->data, (int)sizeof((arr)->data[0])

int oppai_array_reserve_i(int n, int *cap, int *len, void **data, int esize) {
  (void)len;
  if (*cap <= n) {
    void *newdata;
...
void oppai_array_free_i(int *cap, int *len, void **data, int esize) {
  (void)esize;
  free(*data);
  *cap = 0;
  *len = 0;
...
struct ezpp {
  ...
  char buf[0xFFFF];
  int p_flags;
  oppai_array_t(object_t) objects;
  oppai_array_t(timing_t) timing_points;

  /* diffcalc */
  float interval_end;
  float max_strain;
  oppai_array_t(float) highest_strains;

  /* allocator */
  char *block;
  char *end_of_block;
  oppai_array_t(char *) blocks;
};
...
int m_reserve(ezpp_t ez, int min_size) {
  ...
  oppai_array_append(&ez->blocks, ez->block);
  return 1;
}
...
int mods_apply(ezpp_t ez) {
  ...
  arms = ez->ar <= 5 ? (AR0_MS - AR_MS_STEP1 * (ez->ar - 0))
                     : (AR5_MS - AR_MS_STEP2 * (ez->ar - 5));
  arms = al_min(AR0_MS, al_max(AR10_MS, arms));
  arms /= ez->speed_mul;
  ez->ar = arms > AR5_MS ? (0 + (AR0_MS - arms) / AR_MS_STEP1)
...
float playfield_center[] = {PLAYFIELD_WIDTH / 2.0f, PLAYFIELD_HEIGHT / 2.0f};
...
int p_timing(ezpp_t ez, slice_t *line) {
  ...
  int err = 0;
  slice_t split[8];
  timing_t *t = oppai_array_alloc(&ez->timing_points);
...
int p_objects(ezpp_t ez, slice_t *line) {
  ...
  o = oppai_array_alloc(&ez->objects);
...
#undef s
  ...
  for (i = 0; i < ez->objects.len; ++i) {
    ...
    o->tick_spacing =
        al_min(t->beat_len / ez->tick_rate, o->duration / o->repetitions);
    ...
    case MODE_TAIKO: {
      if (o->slider_is_drum_roll && ez->mode != ez->original_mode) {
        ez->max_combo +=
            (int)(ceil((o->duration + o->tick_spacing / 8) / o->tick_spacing));
      }
      break;
    }
    case MODE_STD:
      ...
      ticks = (int)ceil((num_beats - 0.1f) / o->repetitions * ez->tick_rate);
      --ticks;
 ...
 */
float d_spacing_weight(float distance, float delta_time, float prev_distance,
                       float prev_delta_time, float angle, int type,
                       int *is_single) {
  float angle_bonus;
  ...
  case DIFF_SPEED: {
    ...
    *is_single = distance > SINGLE_SPACING;
    ...
    if (delta_time < MIN_SPEED_BONUS) {
      speed_bonus += (float)pow((MIN_SPEED_BONUS - delta_time) / 40.0f, 2);
    }
    angle_bonus = 1.0f;
    ...
    return ((1 + (speed_bonus - 1) * 0.75f) * angle_bonus *
            (0.95f +
             speed_bonus * (float)pow(distance / SINGLE_SPACING, 3.5))) /
           strain_time;
  }
  case DIFF_AIM: {
    float result = 0;
    ...
    if (!is_nan(angle) && angle > AIM_ANGLE_BONUS_BEGIN) {
      angle_bonus = (float)sqrt(al_max(prev_distance - ANGLE_BONUS_SCALE, 0) *
    ...
    return al_max(result + weighted_distance /
...
int d_update_max_strains(ezpp_t ez, float decay_factor, float cur_time,
                         float prev_time, float cur_strain, float prev_strain,
                         int first_obj) {
  /* make previous peak strain decay until the current obj */
  while (cur_time > ez->interval_end) {
    if (!oppai_array_append(&ez->highest_strains, ez->max_strain)) {
      return ERR_OOM;
    }
    if (first_obj) {
      ez->max_strain = 0;
      ...
      float decay;
      decay =
          (float)pow(decay_factor, (ez->interval_end - prev_time) / 1000.0f);
...
int d_std(ezpp_t ez) {
  ...
  ez->speed_length_bonus =
      d_length_bonus(ez->speed_stars, ez->speed_difficulty);
...
int d_taiko(ezpp_t ez) {
  ...
  for (i = 0; i < ez->nobjects; ++i) {
    ...
    if (o->type & OBJ_SLIDER) {
       ...
       */
      for (j = o->time; j < o->time + o->duration + o->tick_spacing / 8;
           j += o->tick_spacing) {
...
void taiko_acc_round(float acc_percent, int nobjects, int nmisses, int *n300,
                     int *n150) {
  int max300;
  float maxacc;
  ...
  acc_percent = al_max(0.0f, al_min(maxacc, acc_percent));
  /* just some black magic maths from wolfram alpha */
  *n150 = (int)al_round(-2.0f *
                        ((acc_percent * 0.01f - 1.0f) * nobjects + nmisses));
...
int pp_std(ezpp_t ez) {
  ...
  float length_bonus =
      (0.95f + 0.4f * al_min(1.0f, nobjects_over_2k) +
       (ez->nobjects > 2000 ? (float)log10(nobjects_over_2k) * 0.5f : 0.0f));

  float miss_penality_aim =
      0.97 * pow(1 - pow((double)ez->nmiss / ez->nobjects, 0.775), ez->nmiss);
  float miss_penality_speed =
      0.97 * pow(1 - pow((double)ez->nmiss / ez->nobjects, 0.775f),
                 pow(ez->nmiss, 0.875f));
  ...
  case 1:
    real_acc = acc_calc(al_max(0, ez->n300 - ez->nsliders - ez->nspinners),
  ...
  ez->aim_pp *=
      1.0f + (float)al_min(ar_bonus, ar_bonus * (ez->nobjects / 1000.0f));
  ...
  ez->speed_pp *= (0.95f + od_squared / 750) *
                  (float)pow(accuracy, (14.5 - al_max(ez->od, 8)) / 2);

  /* it's important to also consider accuracy difficulty when doing that */
  ez->speed_pp *= (float)pow(0.98f, ez->n50 < ez->nobjects / 500.0f
                                        ? 0.00
                                        : ez->n50 - ez->nobjects / 500.0f);
  ...
  ez->pp = (float)(pow(pow(ez->aim_pp, 1.1f) + pow(ez->speed_pp, 1.1f) +
                           pow(ez->acc_pp, 1.1f),
                       1.0f / 1.1f) *
                   final_multiplier);
...
int pp_taiko(ezpp_t ez) {
  ...
  if (ez->max_combo > 0) {
    ez->speed_pp *= (al_min((float)pow(ez->combo - ez->nmiss, 0.5f) /
                                (float)pow(ez->max_combo, 0.5f),
                            1.0f));
  }
  ...
  ez->pp = ((float)pow(pow(ez->speed_pp, 1.1f) + pow(ez->acc_pp, 1.1f),
                       1.0f / 1.1f) *
            final_multiplier);
...
int calc(ezpp_t ez) {
  ...
  if (ez->accuracy_percent >= 0) {
    switch (ez->mode) {
    case MODE_STD:
      acc_round(ez->accuracy_percent, ez->nobjects, ez->nmiss, &ez->n300,
                &ez->n100, &ez->n50);
      break;
    case MODE_TAIKO:
      taiko_acc_round(ez->accuracy_percent, ez->max_combo, ez->nmiss, &ez->n300,
                      &ez->n100);
      break;
    }
  }
...
ezpp_t ezpp_new(void) {
  ...
  if (ez) {
    ...
    ez->base_ar = ez->base_od = ez->base_cs = ez->base_hp = -1;
    oppai_array_reserve(&ez->objects, 600);
    oppai_array_reserve(&ez->timing_points, 16);
    oppai_array_reserve(&ez->highest_strains, 600);
  }
  return ez;
}
...
void ezpp_free(ezpp_t ez) {
  free_owned_map(ez);
  oppai_array_free(&ez->objects);
  oppai_array_free(&ez->timing_points);
  oppai_array_free(&ez->highest_strains);
  m_free(ez);
  free(ez);
}
...
OPPAIAPI int ezpp_ntiming_points(ezpp_t ez) { return ez->timing_points.len; }
...
setter(float, aim_stars) setter(float, speed_stars) setter(float, base_ar)
    setter(float, base_od) setter(float, base_hp) setter(int, mode)
        setter(int, combo) setter(int, score_version)
...
clobber_setter(float, base_cs) clobber_setter(int, mode_override)
...
#define acc_clobber_setter(t, x)                                               \
  ...
  void ezpp_set_##x(ezpp_t ez, t x) {                                          \
    ...
    acc_clobber_setter(int, nmiss) acc_clobber_setter(int, end)
